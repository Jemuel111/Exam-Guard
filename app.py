"""
ExamGuard v2.0 — AI-Powered Online Exam Monitoring System
New in v2: JWT auth, answer submission + auto-grading, exam scheduling,
           audio anomaly hooks, email report delivery, analytics API,
           per-student exam assignment, real-time SocketIO push to teacher
"""
...
def auth_required(role=None):
    """Decorator factory — validates Bearer token."""
    ...
    def decorator(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            # Also accept query param for GET downloads
            if not token:
                token = request.args.get('token', '')
            info = verify_token(token)
            if not info:
                # Demo fallback — allow if demo header present
...
def init_db():
    ...
    # Seed demo exams
    demo_exams = [
        ('Algebra Finals',    'Mathematics',    60, 75, 'Answer all questions carefully. No calculators allowed.', 'active',    None, None),
        ('Chemistry Quiz 3',  'Science',        30, 70, 'Multiple choice only. Show your periodic table knowledge.', 'scheduled', (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d 09:00'), (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d 09:30')),
        ('English Literature','English',        90, 75, 'Read all passages carefully before answering.', 'scheduled', (datetime.now() + timedelta(days=5)).strftime('%Y-%m-%d 13:00'), (datetime.now() + timedelta(days=5)).strftime('%Y-%m-%d 14:30')),
    ]
    for ex in demo_exams:
        c.execute('SELECT id FROM exams WHERE title=?', (ex[0],))
        if not c.fetchone():
            c.execute('''INSERT INTO exams (title,subject,duration_minutes,passing_score,instructions,status,scheduled_start,scheduled_end,created_by)
                         VALUES (?,?,?,?,?,?,?,?,1)''', ex)
            exam_id = c.lastrowid
            if ex[0] == 'Algebra Finals':
                qs = [
                    ('What is the quadratic formula?', 'mc', json.dumps(['x = (-b ± √(b²-4ac)) / 2a','x = -b/2a','x = b²-4ac','x = 2a/b']), 0, 2, 1),
                    ('Simplify: 3x + 2x', 'mc', json.dumps(['5x','6x','x⁵','5x²']), 0, 1, 2),
                    ('What is the slope-intercept form?', 'mc', json.dumps(['y = mx + b','y = ax² + bx + c','y = x/m + b','y = m/x']), 0, 1, 3),
                    ('The sum of angles in a triangle is 180°.', 'tf', json.dumps(['True','False']), 0, 1, 4),
                    ('What is the derivative of x²?', 'mc', json.dumps(['2x','x','x²','2']), 0, 2, 5),
                    ('Solve for x: 2x + 6 = 14', 'fitb', None, None, 1, 6),
                    ('Explain the Pythagorean theorem in your own words.', 'essay', None, None, 3, 7),
                ]
                for q in qs:
                    c.execute('INSERT INTO questions (exam_id,question_text,question_type,choices,correct_answer,points,order_num) VALUES (?,?,?,?,?,?,?)',
                               (exam_id,) + q)
...
class ExamSession:
    ...
    def log_violation(self, vtype, details='', severity='medium'):
        ...
        # Push real-time to teacher room
        socketio.emit('violation', {'session_id': self.session_id,
                                    'student_name': self.student_name,
                                    'violation': entry}, room='teachers')
        return entry
    ...
    def save_reports(self):
        ...
        with open(f'reports/{sid}_violations.csv', 'w', newline='') as f:
            ...
            w.writeheader(); w.writerows(self.violations)
...
def grade_submission(exam_id: int, answers: dict, db) -> dict:
    """Auto-grades MC/TF/FITB; flags essay for manual review."""
    ...
    for q in questions:
        ...
        if q['question_type'] in ('mc', 'tf'):
            try:
                if int(student_ans) == q['correct_answer']:
                    earned = q['points']; feedback = 'Correct'
                else:
                    feedback = 'Incorrect'
            except (ValueError, TypeError):
                feedback = 'Not answered'
        elif q['question_type'] == 'fitb':
            if student_ans and student_ans.strip():
                earned = q['points']; feedback = 'Submitted (manual review)'
            else:
                feedback = 'Not answered'
        else:  # essay
            if student_ans and len(str(student_ans).strip()) > 10:
                earned = q['points']; feedback = 'Submitted (manual review)'
            else:
                feedback = 'Not answered'
        score += earned
        breakdown.append({'question_id': q['id'], 'text': q['question_text'][:60],
                          'earned': earned, 'max': q['points'], 'feedback': feedback})
...
def process_frame(session: ExamSession, frame_b64: str):
    try:
        ...
        if face_count == 0:
            ...
            if session.no_face_start is None: session.no_face_start = now
            ...
            session.no_face_start = None; session._no_face_logged = False
        # Multiple faces
        if face_count > 1:
            session.stats['multiple_face_frames'] += 1
            if session.multi_face_start is None: session.multi_face_start = now
            ...
            session.multi_face_start = None; session._multi_face_logged = False
        # Gaze
        if face_count == 1:
            if analyze_gaze(frame, faces[0]):
                session.stats['look_away_frames'] += 1
                if session.look_away_start is None: session.look_away_start = now
                ...
                session.look_away_start = None; session._look_away_logged = False
        return {'status': 'ok', 'alerts': alerts,
                'violation_count': len(session.violations),
                'stats': session.stats, 'risk': session.compute_risk(),
...
def send_report_email(to_email: str, student_name: str, session_id: str, report: dict):
    """Non-blocking best-effort email delivery."""
    ...
    try:
        ...
        <hr>
        <small>⚠️ This is a decision-support tool. Human review required before any action.</small>
        """
...
def report(session_id):
    ...
    if row:
        viols = db.execute('SELECT * FROM violations WHERE session_id=? ORDER BY elapsed_seconds', (session_id,)).fetchall()
        sub   = db.execute('SELECT * FROM exam_submissions WHERE session_id=?', (session_id,)).fetchone()
        report_data = {
            'session_id': row['session_id'], 'student_name': row['student_name'],
            'exam_date': row['created_at'][:10], 'start_time': row['start_time'],
            'end_time': row['end_time'], 'duration_minutes': row['duration_minutes'],
...
def login():
    ...
    if is_reg:
        ...
        try:
            db.execute('INSERT INTO users (email,password,role,name,student_id,avatar_initials) VALUES (?,?,?,?,?,?)',
    ...
    # Try hashed password first, then plain (legacy demo)
    ...
    return jsonify({'success': True, 'token': 'demo-token', 'role': role})
...
def get_students():
    db = get_db()
    rows = db.execute("SELECT id,name,email,student_id,avatar_initials,created_at FROM users WHERE role='student'").fetchall()
...
def delete_student(sid):
    get_db().execute('DELETE FROM users WHERE id=? AND role="student"', (sid,))
    get_db().commit()
    return jsonify({'success': True})

@app.route('/api/students/<int:sid>/stats', methods=['GET'])
def student_stats(sid):
    db = get_db()
    sessions = db.execute('SELECT * FROM sessions WHERE student_id=? ORDER BY created_at DESC', (sid,)).fetchall()
    submissions = db.execute('SELECT * FROM exam_submissions WHERE student_id=? ORDER BY submitted_at DESC', (sid,)).fetchall()
    return jsonify({
...
def get_exams():
    ...
    for e in exams:
        ex = dict(e)
        ex['question_count'] = db.execute('SELECT COUNT(*) FROM questions WHERE exam_id=?', (e['id'],)).fetchone()[0]
        ex['enrolled_count'] = db.execute('SELECT COUNT(*) FROM exam_enrollments WHERE exam_id=?', (e['id'],)).fetchone()[0]
        result.append(ex)
...
def get_exam(exam_id):
    ...
    if not exam:
        return jsonify({'error': 'Not found'}), 404
    questions = db.execute('SELECT * FROM questions WHERE exam_id=? ORDER BY order_num', (exam_id,)).fetchall()
...
def create_exam():
    data = request.json or {}
    db = get_db()
    c = db.execute('''INSERT INTO exams (title,subject,duration_minutes,passing_score,
    ...
    for i, q in enumerate(data.get('questions',[])):
        db.execute('''INSERT INTO questions (exam_id,question_text,question_type,choices,correct_answer,points,order_num)
                      VALUES (?,?,?,?,?,?,?)''',
                   (exam_id, q['text'], q.get('type','mc'),
                    json.dumps(q.get('choices',[])), q.get('correct_answer',0),
                    q.get('points',1), i+1))
    # Auto-enroll if student IDs provided
    for sid in data.get('enroll_students', []):
        try:
            db.execute('INSERT INTO exam_enrollments (exam_id,student_id) VALUES (?,?)', (exam_id, sid))
        except sqlite3.IntegrityError:
            pass
    db.commit()
...
def update_exam(exam_id):
    ...
    db.execute('''UPDATE exams SET title=?,subject=?,duration_minutes=?,passing_score=?,
               ...
               (data.get('title'), data.get('subject'), data.get('duration_minutes'),
                ...
                data.get('scheduled_start'), data.get('scheduled_end'), exam_id))
    db.commit()
    return jsonify({'success': True})
...
def submit_exam():
    ...
    result = grade_submission(exam_id, answers, db) if exam_id else \
             {'score': 0, 'max_score': 0, 'percentage': 0, 'breakdown': []}
    ...
    db.execute('''INSERT INTO exam_submissions
                  ...
                  VALUES (?,?,?,?,?,?,?,?,datetime('now'))''',
               (session_id, exam_id, student_id, json.dumps(answers),
...
def get_sessions():
    ...
    rows = db.execute('''SELECT s.*, COUNT(v.id) as vcount
                         FROM sessions s LEFT JOIN violations v ON s.session_id=v.session_id
                         GROUP BY s.session_id ORDER BY s.created_at DESC LIMIT 50''').fetchall()
    return jsonify([dict(r) for r in rows])
...
def start_session():
    ...
    db = get_db()
    db.execute('''INSERT INTO sessions (session_id,student_id,exam_id,student_name,start_time,is_active,stats)
                  VALUES (?,?,?,?,?,1,?)''',
...
def audio_alert():
    """Client reports suspicious audio level."""
    data    = request.json or {}
    session = exam_sessions.get(data.get('session_id'))
    if not session:
        return jsonify({'error': 'not found'}), 404
...
def session_status(session_id):
    ...
    if not session:
        return jsonify({'error': 'not found'}), 404
    return jsonify({'session_id': session_id, 'is_active': session.is_active,
                    'elapsed': round(session.get_elapsed()),
                    'violation_count': len(session.violations),
                    'stats': session.stats, 'risk': session.compute_risk()})
...
@app.route('/api/analytics/violations_by_type', methods=['GET'])
...
def risk_distribution():
    db   = get_db()
    rows = db.execute("SELECT risk_level, COUNT(*) as count FROM sessions WHERE is_active=0 GROUP BY risk_level").fetchall()
    return jsonify([dict(r) for r in rows])

@app.route('/api/analytics/daily_sessions', methods=['GET'])
def daily_sessions():
    db   = get_db()
    rows = db.execute('''SELECT DATE(created_at) as day, COUNT(*) as count
                         FROM sessions GROUP BY day ORDER BY day DESC LIMIT 14''').fetchall()
    return jsonify([dict(r) for r in rows])

@app.route('/api/notifications', methods=['GET'])
def get_notifications():
    db = get_db()
    # Return recent high-risk sessions as notifications
    rows = db.execute("""SELECT 'HIGH_RISK_SESSION' as type,
                          student_name || ' flagged High Risk (' || risk_score || ')' as message,
                          created_at FROM sessions WHERE risk_level='High'
...
def download_report(session_id):
    ...
    return send_file(path, as_attachment=True, download_name=f'{session_id}_report.json')
...
def download_csv(session_id):
    ...
    return send_file(path, as_attachment=True, download_name=f'{session_id}_violations.csv')
...
if __name__ == '__main__':
    init_db()
    logger.info('ExamGuard v2.0 starting at http://localhost:5000')
